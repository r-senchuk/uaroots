...
| `lviv-hamburg` | Same Lviv hub. | Verified: Гамбург is in the Schleswig-Holstein / Hamburg group. | `koval-de` | Thin; page says commercial status unconfirmed | Page exists if URL is known; homepage search does **not** navigate here | No (`noindex`) | **editorial** | Keep out of `/routes/` and sitemap. |
| `lviv-berlin` | Same Lviv hub. | Verified: Берлін is in the Brandenburg / Berlin group. | `koval-de` | Thin; unconfirmed commercial status | Page exists if URL is known; homepage search does **not** navigate here | No | **editorial** | Same as Hamburg. |
